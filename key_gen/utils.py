...
from dict_use import dict_use_c_k
dict_use_c_k=dict_use_c_k(config.tdict_path)
sen2id_c_k=dict_use_c_k.sen2id
id2sen_c_k=dict_use_c_k.id2sen
...
#id列変換関数(4.1.2)
def sequence_change(input):
    input_idlist = input.tolist()
    input_senlist = id2sen(input_idlist)
    sentence = ''.join(input_senlist).decode('utf-8')
    output_senlist = [char.encode('utf-8') for char in sentence]
    output_idlist = [config.tdict_size+2]
    output_idlist.extend(sen2id_c_k(output_senlist))
    if len(output_idlist) > config.num_steps:
        return np.array([output_idlist[:config.num_steps]]).astype(np.int32), [config.num_steps]
    else:
        return np.concatenate([np.array(output_idlist).astype(np.int32),np.full(config.num_steps - len(output_idlist), config.tdict_size+1).astype(np.int32)], 0)[np.newaxis, :], [len(output_idlist)]

#id列変換関数（複数文に適用する場合）
def sequence_change_multi(input):
    output_length = []
    for i in range(len(input)):
        input_idlist = input[i].tolist()
        input_senlist = id2sen(input_idlist)
        sentence = ''.join(input_senlist).decode('utf-8')
        output_senlist = [char.encode('utf-8') for char in sentence]
        output_idlist = [config.tdict_size+2]
        output_idlist.extend(sen2id_c_k(output_senlist))
        if len(output_idlist) > config.num_steps:
            i_idlist = np.array([output_idlist[:config.num_steps]]).astype(np.int32)
            output_length.append(config.num_steps)
        else:
            i_idlist = np.concatenate([np.array(output_idlist).astype(np.int32),np.full(config.num_steps - len(output_idlist), config.tdict_size+1).astype(np.int32)], 0)[np.newaxis, :]
            output_length.append(len(output_idlist))
        if i == 0:
            new_idlist = i_idlist
        else:
            new_idlist = np.concatenate([new_idlist, i_idlist], 0)
    return new_idlist, output_length